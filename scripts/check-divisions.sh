...
# runbooks (no agent frontmatter), not agents. agno-demo/ and final-delivery/
# are app/demo artifacts and reports, not source-agent categories. None of them
# are divisions — they must never be scanned as source-agent categories.
NON_DIVISION_DIRS=(examples scripts integrations strategy agno-demo final-delivery)
